...
export const HandleNewProduct = async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("Error en HandleCrearProducto:", (error as Error).message);
    res.status(500).json({ error: "Error interno del servidor." });
  }
};

export const HandleGetProductsByUser = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ msg: "Error al obtener productos del usuario.", error: (error as Error).message });
  }
};
...
export const HandleGetProductById = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ msg: "Error interno del servidor.", error: (error as Error).message });
  }
};

export const HandleGetProductsByBrand = async (req: Request, res: Response) => {
  try {
    const { marca } = req.params;
    const productos = await getProductsByBrand(marca);
    res.status(200).json(productos);
  } catch (error) {
    res.status(500).json({ msg: "Error al obtener productos por marca.", error: (error as Error).message });
  }
};

export const HandleGetProductsByType = async (req: Request, res: Response) => {
  try {
    const { tipo } = req.params;
    const productos = await getProductsByType(tipo);
    res.status(200).json(productos);
  } catch (error) {
    res.status(500).json({ msg: "Error al obtener productos por tipo.", error: (error as Error).message });
  }
};

export const HandleGetProductsByBody = async (req: Request, res: Response) => {
  try {
    const { cuerpo } = req.params;
    const productos = await getProductsByBody(cuerpo);
    res.status(200).json(productos);
  } catch (error) {
    res.status(500).json({ msg: "Error al obtener productos por cuerpo.", error: (error as Error).message });
  }
};
