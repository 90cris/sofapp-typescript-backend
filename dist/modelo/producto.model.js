...
export const insertProduct = async (producto) => {
    ...
    if (!nombre || !marca || !tipo || cuerpo === undefined ||
    ...
    const result = await db.query(`INSERT INTO productos (id_usuario, nombre, marca, tipo, cuerpo, alto, ancho, precio, foto, detalle, stock, color)
...
export const getProductsByUser = async (id_usuario) => {
    const { rows } = await db.query("SELECT * FROM productos WHERE id_usuario = $1;", [id_usuario]);
    return rows;
};
...
export const modifyStock = async (id_producto, cantidad) => {
    try {
        const query = `
      UPDATE productos
      SET stock = stock + $1
